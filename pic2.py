...
class background(arcade.Window):
    def __init__(self, width, height):
        ...
        self.horse = arcade.Sprite('images/horse.png')
        self.horse.set_position(150 , 370)
        self.moon = arcade.Sprite('images/moon.png')
        self.moon.set_position(230 , 370)           
        self.t1 = arcade.create_text("ร์    นํ้    ก    น    ย    ด    พ", arcade.color.BLACK, 20)
        self.t2 = arcade.create_text("า    จั     ม้    ก    ย    ท    มื", arcade.color.BLACK, 20)

    def on_draw(self):
        arcade.start_render()
        arcade.draw_rectangle_filled(200,330,290,290, arcade.color.LIGHT_CYAN)
        for i in range(4):
            arcade.draw_rectangle_filled(150+30*i,270,25,30, arcade.color.QUEEN_BLUE)
        self.horse.draw() 
        self.moon.draw() 
        for j in range(7):
